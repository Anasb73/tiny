...
def check_model(model):
  ...
  while idx < len(model.layers):
    if idx == len(model.layers) - 1 or "Lambda" in model.layers[idx].__class__.__name__:
      nn_head = True
    ...
    ## Check isolated activation layers
    if ((not nn_head) and model.layers[idx].__class__.__name__ != "BatchNormalization" and
                          model.layers[idx+1].__class__.__name__ == "Activation"):
      print("ERROR: Activation layer (%s) must be integrated to previous layer" % model.layers[idx+1].name)
      exit()
...
def transform_model(model):
  ...
  while idx < len(model.layers):
    ...
    if model.layers[idx].__class__.__name__ == "Activation" and model.layers[idx-1].__class__.__name__ != "BatchNormalization" and \
       (model.layers[idx-1].__class__.__name__ == "Conv2D" or model.layers[idx-1].__class__.__name__ == "DepthwiseConv2D" or \
        model.layers[idx-1].__class__.__name__ == "Dense"):
